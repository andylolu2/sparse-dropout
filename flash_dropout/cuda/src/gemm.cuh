...
struct SmemGemm {
   ...
   public:
    CUTE_DEVICE SmemGemm(ct::Tensor<Gmem<ct::half_t>, LayoutBlkC> &C_)
        : C(C_), thread_mma(tiled_mma.get_thread_slice(threadIdx.x)), C_frag(thread_mma.partition_fragment_C(C)) {
        ct::clear(C_frag);
    }
    ...
    CUTE_DEVICE void scale(float alpha) {
        CUTE_UNROLL
...
template <typename T, typename SrcLayout, typename DstLayout,
          typename TiledCopy>
CUTE_DEVICE void
load_block_from_gmem_to_smem(const ct::Tensor<Gmem<T>, SrcLayout> &src,
...
// For m = 3, n = 4, group_size_m = 2, produces the coordiantes in the following
// order:
//  |  1 |  3 |  5 |  7 |
//  |  2 |  4 |  6 |  8 |
//  |  9 | 10 | 11 | 12 |
CUTE_DEVICE std::tuple<int64_t, int64_t>
threadblock_swizzle(int64_t idx, int64_t m, int64_t n, int64_t group_size_m) {
...
void gemm(const ct::Tensor<Gmem<ct::half_t>, LayoutA> &A,
    ...
    gemm_kernel<Config><<<block_dim, thread_dim>>>(A, B, C, group_size_m);
}